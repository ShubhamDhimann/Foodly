...
function FoodCard() {
    ...
    const increaseQuantity = (dishID) => setQuantity((qty) => ({ ...qty, [dishID]: (qty[dishID] || 0) + 1 }));
    const decreaseQuantity = (dishID) => setQuantity(quantity > 1 ? quantity - 1 : 1);
                ...
                (allCategories.map((category) => (
                    <div key={category._id} className="mb-12">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                            {allDishes
                                .filter((dish) => (dish.CategoryName === category.CategoryName) && (dish.name.toLowerCase().includes(search.toLowerCase())))
                                .map((dish) => (
                                        ...
                                        <div className="p-4">
                                            ...
                                            <div className="flex items-center justify-between mt-4">
                                                <div className="flex items-center">
                                                    ...
                                                    <span className="px-3">{quantity[dish._id] || 0}</span>
                                                    <button onClick={() => { increaseQuantity(dish._id) }} className="px-2 py-1 border text-xl font-bold rounded hover:bg-gray-100"> + </button>
                                                </div>

                                                <button onClick={(e) => { quantity[dish._id] && handleAddToCart(e, dish._id, dish.name, quantity[dish._id], dish.price) }} className="bg-gray-900 text-white px-4 py-2 rounded-lg hover:bg-gray-800 transition">
                                                    Add to Cart
                                                </button>
                                            </div>
                                        </div>
